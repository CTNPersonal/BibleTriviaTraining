...
QUESTIONS = os.getenv("QUESTIONS", "questions")
...
def initialize():
    ...
    st.markdown(f"**Please tell me who's playing:**")
    st.write("\n")
    ...
    # Load Bible trivia data for a user
    if st.button("Submit Name"):
        print(f"User name: {st.session_state.user}")
        user_questions = [QUESTIONS]
        if st.session_state.user:
            user_section = st.session_state.user.lower()
            user_questions = st.secrets.get(user_section, {}).get(
                "questions", [QUESTIONS]
            )

        print(f"Loading questions from {user_questions}")
        questions = []
        for book in user_questions:
            with open(f"{book}.json") as f:
                data = json.load(f)
